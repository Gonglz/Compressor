/**
 * performance benchmark program - noteompv8noteompv15noteacross different thread countsnote
 * using real ResNet50 data
 */
...
// simplified binary loading(notetest_c_realnote)
typedef struct {
    char layer_name[256];
    size_t shape[8];
    size_t ndim;
...
int main(int argc, char** argv) {
    ...
    // create compressor
    CompressorConfig config = {.momentum_lr = 0.07f,.consistency_threshold = 0.5f,.lossless_compressor = "zstd",.error_bounding_mode = "ABS",.error_bound = 1.0f,.param_count_threshold = 1024,.max_history_length = 3
    };
    ...
    // output results(formatted for script parsing)
