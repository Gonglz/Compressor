...
// ========== Phase 20: 分离的纯计算结果结构 ==========
/**
 * LayerResult: 纯计算函数的返回值
 * 包含压缩后的数据和后续更新hash/history所需的元信息
 * 在并行区内由纯计算函数填充，不修改任何全局状态
 * 在串行区（Step C）中用于更新hash/history/breakdown
 */
typedef struct {
    // [计算结果] 压缩后的数据（核心）
    CompressedLayerData *compressed;      // 仅当成功时非NULL
    
    // [后续更新] 更新prediction_memory需要的信息
    char history_key[512];                // 用于history lookup: make_history_key(client_id, layer_name)
    char layer_key[256];                  // 用于layer memory: make_layer_key_from_shape
    
    // [统计信息] 每层的breakdown数据
    LayerBreakdown layer_bd;              // breakdown详细数据（已填充）
    
    // [状态标志] 计算结果状态
    int status;                           // 0=成功, <0=失败码
    char error_msg[256];                  // 错误信息（如有）
} LayerResult;
...
// 全局 breakdown 开关（默认关闭）
static int g_enable_breakdown = 1;  // ✅ Phase 19sz3.3: 默认启用breakdown统计
...
/**
 * Phase 4优化：融合统计计算（mean + std + min/max）
 * 
 * 优化策略：
 * 1. 一次遍历完成所有统计（原来需要3次）
 * 2. 对大数组(>10K元素)使用OpenMP并行化
 * 3. 使用reduction语句避免原子操作开销
 */
static FloatStats compute_stats_fused(const float *data, size_t size) {
    ...
    // 阈值：大于10K元素才使用OpenMP（避免小数组的线程开销）
    if (size >= 10000) {
        // ✅ Phase 18.1: SIMD优化统计计算
        // 第一遍：并行+SIMD计算 sum, min, max
    ...
    if (!layer_memory) {
        ...
        #pragma omp critical(memory_create)
        {
            ...
            if (!layer_memory) {
                ...
                // 重新获取hash表中的指针（set_prediction_memory_for_layer内部会copy）
...
// ========== Phase 20: 纯计算函数 (Step A) ==========
/**
 * 单层纯计算压缩（不修改全局状态）
 * 
 * ✅ 完全隔离计算，适合OpenMP并行化
 * ❌ 不调用: set_prediction_memory_for_layer, add_gradient_to_history
 * ❌ 不修改: mc->layer_histories, mc->prediction_memories, g_last_batch_breakdown
 * 
 * 返回LayerResult，包含压缩数据和后续更新所需的元信息
 * 这些信息将在并行结束后的Step C中由单线程安全地处理
 * 
 * 参数:
 *   compressor: 压缩器实例
 *   client_id: 客户端ID
 *   layer_name: 层名称
 *   gradient: 梯度数据
 *   out_result: 输出结果指针（由调用者分配的LayerResult）
 * 
 * 返回: 0=成功, <0=失败
 */
static int momentum_compressor_compress_layer_pure(
    ...
    // [Step A1] 生成history_key和layer_key（纯计算，无全局修改）
    ...
    // [Step A2] 获取步数（读取，无修改）
    // 注意: 实际上increment_step_count修改全局状态，但这是必需的
    // 我们需要在Step C中移动它到串行区
    // 暂时在这里保留，最后会改进
    char key[MAX_KEY_LENGTH];
    make_history_key(key, client_id, layer_name);
    
    // [WARNING] 这里应该有thread-safe的step获取
    // 由于increment_step_count修改全局状态，我们需要将其移到串行区
    // 为了简单起见，暂时在这里调用，但这会被后续改进
    // TODO: 将step计数移到Step C
    ...
    if (current_step == 1) {
        ...
        // [问题] 这里尝试读取mc->layer_histories，但在并行区内多线程访问会有竞争
        // 解决: 使用#pragma omp critical来保护读取
        NDArray *prev_grad = NULL;
        
        // 在并行区内需要保护对历史的读取
        // 但这里不能加critical，因为是在pure函数内
        // 实际上应该在Step B中先读取所有需要的previous_grad，然后传入
        // 这是一个架构问题，需要改进
        
        // 暂时在这里用critical读取（不完美，但保证正确性）
...
static NDArray* decompress_momentum_predicted_layer(
    ...
    // 10.5 更新预测记忆（layer_memory已在循环中更新，这里保存回二级结构）
    // 由于layer_memory指向layer_memory_array->data，修改已经生效
    // 但为了代码清晰，我们显式调用set（内部会clone）
    set_prediction_memory_for_layer(compressor, client_id, compressed->shape, 
                                   compressed->ndim, layer_memory_array);
...
int momentum_compressor_compress_batch(
    ...
    // ========== Phase 20: 分离架构实现并行化 ==========
    // ✅ Step A: 纯计算（统计+压缩） + 并行
    // ✅ Step B: OpenMP并行for循环调用pure函数
    // ✅ Step C: 并行后串行更新hash/history/breakdown
    ...
    // [Step B] OpenMP并行调用纯计算函数
    // 各线程独立处理纯计算（统计+SZ3压缩），不涉及全局状态修改
    // v22优化：移除v21的自适应调度预扫描（实测增加15%开销）
    // 直接使用dynamic(1)，兼顾大小层负载均衡
    ...
    // [Step C] 串行更新全局状态（hash表、history、breakdown）
    // 这一步必须是单线程，否则会导致竞争条件和段错误
    for (size_t i = 0; i < num_items; i++) {
        ...
        // [更新1] 更新预测记忆（需要thread-safe，这里单线程安全）
        // 获取预测记忆数组（如果是动量预测层）
        if (strcmp(compressed->type, "momentum_predicted") == 0) {
            // 从compressed中获取更新后的预测记忆
            // 通常在compress_conv_layer_momentum中已经计算
            // 这里需要存储回预测记忆数据库
            // [TODO] 实现预测记忆的持久化
            set_prediction_memory_for_layer(
                compressor, client_id,
                compressed->shape, compressed->ndim,
                NULL  // [FIXME] 需要从compressed中提取prediction_memory
            );
        }
        
        // [更新2] 梯度已经在compress_*_layer中添加到历史
        // 这里无需重复添加（纯计算函数不修改历史，但compress_*_layer内部会）
