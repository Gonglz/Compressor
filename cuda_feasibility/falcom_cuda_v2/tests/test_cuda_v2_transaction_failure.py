...
def main() -> None:
    ...
    state = FalcomCudaV2State(client_id="txn", config=default_config())
    x0 = torch.ones((64, 64, 4, 4), device="cuda", dtype=torch.float32)
    x1 = (x0 + 0.01).contiguous()
    compress_batch_cuda_v2(OrderedDict([("conv.weight", x0)]), state_handle=state, options={"cuda_min_numel": 1})
    ...
    print("ok - cuda v2 transaction failure leaves state unchanged")
