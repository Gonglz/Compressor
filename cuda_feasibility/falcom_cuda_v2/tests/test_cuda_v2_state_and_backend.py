...
def main() -> None:
    ...
    state = FalcomCudaV2State(client_id="state_backend", config=default_config())
    ...
    compress_batch_cuda_v2(OrderedDict([("convA.weight", a0), ("convB.weight", b0)]), state_handle=state, options={"cuda_min_numel": 1})
    compress_batch_cuda_v2(OrderedDict([("convA.weight", a1), ("convB.weight", b1)]), state_handle=state, options={"cuda_min_numel": 1})
    ...
    cpu_owned = FalcomCudaV2State(client_id="owner_stable", config=default_config())
    small = torch.ones((8,), device="cuda", dtype=torch.float32)
    compress_batch_cuda_v2(OrderedDict([("small.bias", small)]), state_handle=cpu_owned, options={"cuda_min_numel": 1024})
    key_small = layer_key("small.bias", small)
    assert cpu_owned.backend_by_key[key_small] == "cpu"
    compress_batch_cuda_v2(OrderedDict([("small.bias", small)]), state_handle=cpu_owned, options={"cuda_min_numel": 1})
    assert cpu_owned.backend_by_key[key_small] == "cpu"
    print("ok - cuda v2 state and backend ownership")
