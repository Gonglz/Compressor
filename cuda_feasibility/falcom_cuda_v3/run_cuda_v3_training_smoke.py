#!/usr/bin/env python3
"""Replay-based CUDA v3 training smoke proxy.

The transferred bundle contains real gradient/model rounds, not a full
train/eval dataloader.  This smoke therefore compares CPU safe grouped and
CUDA v3 q8 on the same replay rounds and records training-adjacent signals:
round time, compression ratio, gradient norm, and reconstruction error.
"""
...
def gpu_round(
    c_state: FalcomCudaV3State,
    d_state: FalcomCudaV3State,
    model: OrderedDict,
    original_bytes: int,
    threshold: int,
    quant_mode: str,
    ...
    safe_sync()
    t0 = time.perf_counter()
    layers, _stats = compress_batch_cuda_v3(
        model,
        state_handle=c_state,
        options={"cuda_min_numel": threshold, "quant_mode": quant_mode},
    )
    payload = dumps_cuda_v3_layers(layers)
    safe_sync()
    t1 = time.perf_counter()
    decoded, _dstats = decompress_batch_cuda_v3(loads_cuda_v3_layers(payload), state_handle=d_state)
    safe_sync()
    t2 = time.perf_counter()
    m = metrics(model, decoded)
    return {
...
def main() -> None:
    ...
    parser.add_argument("--quant-mode", default="8")
    ...
    for model_name in args.models:
        ...
        gpu_c_state = FalcomCudaV3State(client_id=f"{model_name}_smoke_v3", config=config())
        gpu_d_state = FalcomCudaV3State(client_id=f"{model_name}_smoke_v3", config=config())
        ...
        for idx in range(args.rounds):
            ...
            for result in (
                ...
                gpu_round(gpu_c_state, gpu_d_state, model, original_bytes, args.threshold, str(args.quant_mode)),
            ):
    ...
    with report.open("w") as f:
        f.write("# CUDA v3 Training Smoke Proxy\n\n")
