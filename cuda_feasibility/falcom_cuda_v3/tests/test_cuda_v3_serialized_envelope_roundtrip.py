...
def main() -> None:
    ...
    c_state = FalcomCudaV3State(client_id="serialized", config=default_config())
    d_state = FalcomCudaV3State(client_id="serialized", config=default_config())
    ...
    blob = dumps_cuda_v3_layers(layers)
    loaded = loads_cuda_v3_layers(blob)
    assert isinstance(loaded, OrderedDict)
    out, dstats = decompress_batch_cuda_v3(loaded, state_handle=d_state)
