...
// note legacy [BREAKDOWN] note.Release hot path defaultnote; note
// FALCOM_LEGACY_BREAKDOWN=1 note momentum_compressor_enable_breakdown(1) note.
...
static CompressedLayerData* compress_conv_layer_momentum(
    ...
    // noteshared memorynote(note, note).Batch grouped path sets a
    ...
    if (!layer_memory &&!g_local_prediction_memory_override) {
        ...
        #pragma omp critical(memory_create)
        {
            ...
            if (!layer_memory) {
                ...
                // notehashnote(set_prediction_memory_for_named_layernotecopy)
